...
- log in to a remote machine and execute commands - ex 1 - simple login: `ssh ty@192.168.30.120` - ex 2 - login and execute a series of arbitrary commands:
...
- control the systemd system and service manager
  ...
  - `systemctl status <service>` - check status of a service
    ...
    `systemctl enable <service>` - enable a service (make it run on boot):
...
- non-interactive download of files from the Web - `wget -O- https://sourceforge.net/projects/bacula/files/bacula/9.0.1/bacula-regress-9.0.1.tar.gz/download | tar -xzvf - -C /usr/src` - download a .tar and unzip to to the `/usr/src` directory - ‘O’ flag specifies that its a file to download
  `who is`
